...
<form class="form-horizontal" action=existvmsredhat.py method='POST'>
<div class="container">
<div class="jumbotron">
<center><h1>REDHAT OS</h1></center>
</div>
<div class="container">
 <div class="col-md-4">
...
if(port=="None"):
	...
	if(option=='pu'):
		pu=subprocess.Popen("sudo virsh start redhat"+str(port)+"",shell=True,stdout=subprocess.PIPE)
		pu2=pu.stdout.readline()
		if(pu2==10):
			print("<script>")
			print("alert('Note: You Don't need to POWER UP after selected RAM and CPU');")
			print("</script>")
	elif(option=='po'):
		po=subprocess.Popen("sudo virsh destroy redhat"+str(port)+"",shell=True,stdout=subprocess.PIPE)
		po2=po.stdout.readline()
	elif(option=='rs'):
		subprocess.Popen("sudo virsh destroy redhat"+str(port)+"",shell=True,stdout=subprocess.PIPE)
		subprocess.Popen("sudo virsh start redhat"+str(port)+"",shell=True,stdout=subprocess.PIPE)
	#	subprocess.Popen("sudo virsh start redhat"+str(port)+"",shell=True)
	elif(option=='res'):
		subprocess.Popen("sudo virsh start redhat"+str(port)+"",shell=True,stdout=subprocess.PIPE)
		subprocess.Popen("sudo virsh reset redhat"+str(port)+"",shell=True,stdout=subprocess.PIPE)
		

#status checking...
print("<br/><br/>")
print("Status:")
ab=subprocess.Popen("sudo virsh domcontrol redhat"+str(port)+" &",shell=True,stdout=subprocess.PIPE)
ba=ab.stdout.readline()
if(ba[0]==111):
	print("<font color='green'>Poweron</font>")
	print("<a href=/cgi-bin/existvmsredhat.py>Recheck</a>")
