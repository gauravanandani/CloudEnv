...
email=f1.read()
cursor.execute("update registration set	port2='"+str(i)+"',mport='"+str(p)+"' where email='"+str(email)+"'")
conn.commit()
conn.close()
...
#building process
subprocess.Popen("sudo qemu-img  create -f qcow2 -b  /var/lib/libvirt/images/ubuntu.qcow2 /var/lib/libvirt/images/ubuntu"+str(i)+".qcow2 ",shell=True,stdout=subprocess.PIPE)

f1=open("files/psd","r")
p=f1.read()
subprocess.Popen("sudo virt-install --name ubuntu"+str(i)+" --ram "+str(ram)+" --vcpu "+str(cpu)+" --disk path=/var/lib/libvirt/images/ubuntu"+str(i)+".qcow2 --import --graphics=vnc,listen=127.0.0.1,port="+str(r)+",password="+str(p)+" --noautoconsole  &",shell=True,stdout=subprocess.PIPE)
